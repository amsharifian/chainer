...
def ExpandGEP(ng):
	...
	for n in nx.nodes(ng):
		if ng.node[n]['opcode'] != 'GetElementPtr':
			continue
		#print ng.node[n]
		l = []
...
def RemoveDataNodes(ng):
	for n in nx.nodes(ng):
		if ng.node[n]['color'] == 'black':
			continue
		if ng.node[n]['color'] == 'red':
			continue
		if ng.node[n]['color'] == 'blue':
...
def GetDirectedEdge(ng, (src, dst)):
	s = Uncopy(src)
	d = Uncopy(dst)
...
def ExtendChain((src,dst), (u,v), chain, chains, s, g, dag):
	s.remove((u,v))
	RemoveEdge(g, u, v)
	# if src == '1':
		# return
